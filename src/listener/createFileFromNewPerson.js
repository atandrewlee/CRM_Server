...
export function createNewUserUpdate(req, res) {
    ...
    if (before_row.Name === null && after_row.Name !== null && typeof(after_row.Name) === 'string') {
        res.send(dropboxUploadFile(process.env.CRM_FILE_PATH + after_row.Name + ".md", STARTING_CRM_PAGE_TEMPLATE))
    }
}
...
export function createNewUserInsert(req, res) {
    ...
    if (row.Name !== null) {
        res.send(dropboxUploadFile(process.env.CRM_FILE_PATH + row.Name + ".md"), STARTING_CRM_PAGE_TEMPLATE);
    }
}
